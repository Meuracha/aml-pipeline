...
airflow = pytest.importorskip("airflow", reason="airflow not installed locally — run in Docker")
...
class TestDAGStructure:
    ...
    def test_bronze_dag_id(self):
        assert self.bronze_dag.dag_id == 'aml_bronze_pipeline'

    def test_silver_dag_id(self):
        assert self.silver_dag.dag_id == 'aml_silver_pipeline'

    def test_gold_dag_id(self):
        assert self.gold_dag.dag_id == 'aml_gold_pipeline'
    ...
    def test_ml_dag_has_all_tasks(self):
        ...
        required = {'prepare_dataset', 'train_model', 'evaluate_model', 'register_model'}
