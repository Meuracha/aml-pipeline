...
class TestBronzeDataQuality:
    ...
    def test_required_columns_exist(self, sample_raw_df):
        required = [
            ...
            'To Bank', 'Account.1', 'Amount Received',
    ...
    def test_no_negative_amounts(self, sample_raw_df):
        assert (sample_raw_df['Amount Received'] >= 0).all(), \
            "Negative Amount Received found"
        assert (sample_raw_df['Amount Paid'] >= 0).all(), \
            "Negative Amount Paid found"
    ...
    def test_payment_format_valid(self, sample_raw_df):
        valid_formats = {'ACH', 'Wire', 'Cash', 'Cheque',
                         'Credit Card', 'Bitcoin', 'Reinvestment'}
...
class TestSilverDataQuality:
    ...
    def test_no_null_critical_columns(self, sample_silver_df):
        ...
        for col in critical:
            assert sample_silver_df[col].notna().all(), \
                f"Null values in critical column: {col}"

    def test_transaction_id_unique(self, sample_silver_df):
        assert sample_silver_df['transaction_id'].nunique() == len(sample_silver_df), \
    ...
    def test_account_masked_format(self, sample_silver_df):
        ...
        receiver_ok = sample_silver_df['receiver_account_masked'].str.match(pattern).all()
    ...
    def test_amount_positive(self, sample_silver_df):
        assert (sample_silver_df['amount'] > 0).all(), \
            "Non-positive amounts found in Silver"

    def test_timestamp_dtype(self, sample_silver_df):
        assert pd.api.types.is_datetime64_any_dtype(sample_silver_df['timestamp']), \
...
class TestGoldDataQuality:
    ...
    FEATURE_COLS = [
        ...
        'is_high_risk_type', 'is_structuring', 'is_round_amount',
        'rule_score', 'is_laundering'
    ]
...
class TestDataContract:
    ...
    def test_schema_consistency_bronze_to_silver(self,
                                                  sample_raw_df,
                                                  sample_silver_df):
        """Row count should be preserved (or documented why not)"""
        assert len(sample_silver_df) <= len(sample_raw_df), \
            "Silver should have <= rows than Bronze (dead letter removes invalid)"

    def test_laundering_label_preserved(self,
                                         sample_silver_df,
                                         sample_gold_df):
        ...
        assert abs(silver_rate - gold_rate) < 0.01, \
