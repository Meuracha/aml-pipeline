...
def get_alerts(status=None, typology=None, limit=500,
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigation",
        ["📊 Executive Summary", "🚨 Alert Management",
         "📈 Risk Analytics", "🤖 Model Performance",
         "🔎 Transaction Search"],
...
if page == "📊 Executive Summary":
    ...
    with col2:
        st.metric("Laundering Cases", f"{summary.get('total_laundering', 0):,}",
                  delta=f"{summary.get('laundering_rate_pct', 0):.4f}%")
    with col3:
        st.metric("Total Alerts", f"{summary.get('total_alerts', 0):,}")
    with col4:
        st.metric("Open Alerts", f"{summary.get('open_alerts', 0):,}",
                  delta_color="inverse")
    with col5:
    ...
    if daily_data:
        ...
        with col1:
            date_from = st.date_input("From", value=date(2022, 9, 1),
                                       min_value=date(2022, 9, 1),
                                       max_value=date(2022, 9, 17))
        with col2:
            date_to = st.date_input("To", value=date(2022, 9, 17),
                                     min_value=date(2022, 9, 1),
                                     max_value=date(2022, 9, 17))

        mask = (daily_df['date'].dt.date >= date_from) & (daily_df['date'].dt.date <= date_to)
        filtered_daily = daily_df[mask]

        col1, col2 = st.columns(2)
        with col1:
            fig = px.line(filtered_daily, x="date", y="transaction_count",
                          title="Daily Transaction Volume", markers=True)
            fig.update_layout(height=300)
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            fig = px.bar(filtered_daily, x="date", y="alert_count",
                         title="Daily Alert Count",
                         color="alert_count", color_continuous_scale="Reds")
    ...
    with col2:
        ...
        if dist.get("histogram"):
            ...
            fig = px.bar(hist_df, x="bin", y="count",
    ...
    with col1:
        ...
        if typology_data:
            ...
            fig = px.pie(typ_df, values="count", names="typology",
    ...
    with col2:
        ...
        if senders:
            ...
            fig = px.bar(sender_df, x="avg_rule_score", y="sender_account_masked",
                         orientation="h", title="Top 10 High-Risk Senders",
                         color="avg_rule_score", color_continuous_scale="Reds",
                         text="tx_count")
...
# ─────────────────────────────────────────
    ...
    with col1:
        date_from = st.date_input("Alert Date From", value=date(2022, 5, 1),
                                   key="alert_date_from")
    with col2:
        date_to = st.date_input("Alert Date To", value=date(2026, 12, 31),
                                 key="alert_date_to")
    with col3:
    ...
    with st.spinner("Loading alerts..."):
        df = get_alerts(status=status_param, typology=typology_param, limit=limit,
    ...
    with st.expander("⚡ Bulk Update Status"):
        col1, col2 = st.columns(2)
        with col1:
            selected_ids = st.text_area("Alert IDs (one per line)", height=100,
    ...
    display_df = df[['alert_id', 'transaction_id', 'risk_score',
                      'typology', 'status', 'created_at']].copy()
    ...
    with col2:
        new_status_single = st.selectbox("New Status",
    ...
    with col3:
        ...
        if st.button("Update", type="primary"):
            if alert_id_input:
                result = api_patch(f"/alerts/{alert_id_input}",
                                   json={"status": new_status_single})
                if result and result.get("updated"):
...
# ─────────────────────────────────────────
    ...
    with col1:
        ...
        if risk_levels:
            fig = go.Figure(go.Funnel(
                ...
                x=[risk_levels.get("CRITICAL", 0), risk_levels.get("HIGH", 0),
                   risk_levels.get("MEDIUM", 0), risk_levels.get("LOW", 0)],
        ...
        if typology_data:
            ...
            for i, row in typ_df.iterrows():
                fig.add_trace(go.Bar(
                    name=row['typology'], x=[row['typology']], y=[row['count']],
    ...
    if payment_data:
        ...
        col1, col2 = st.columns(2)
        with col1:
            fig = px.bar(pay_df.sort_values("avg_rule_score", ascending=True),
                         x="avg_rule_score", y="payment_type", orientation="h",
                         title="Avg Risk Score by Payment Type",
                         color="avg_rule_score", color_continuous_scale="RdYlGn_r",
                         text="tx_count")
            fig.update_traces(texttemplate="%{text:,}", textposition="outside")
            fig.update_layout(height=400, coloraxis_showscale=False)
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            fig = px.bar(pay_df.sort_values("laundering_rate", ascending=True),
                         x="laundering_rate", y="payment_type", orientation="h",
                         title="Laundering Rate by Payment Type (%)",
                         color="laundering_rate", color_continuous_scale="Reds")
            fig.update_layout(height=400, coloraxis_showscale=False)
            st.plotly_chart(fig, use_container_width=True)
    ...
    if senders:
        sender_df = pd.DataFrame(senders)
        st.dataframe(sender_df.rename(columns={
...
# ─────────────────────────────────────────
    ...
    with col2:
        st.metric("Val AUC-PR", "0.0066", help="Expected low due to 0.05% positive rate")
    with col3:
    ...
    with col1:
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(name="Our Model", x=metrics_df["Metric"], y=metrics_df["Val"],
                             marker_color="#2196F3",
                             text=[f"{v:.4f}" for v in metrics_df["Val"]],
                             textposition="outside"))
        fig.add_trace(go.Bar(name="Random Baseline", x=metrics_df["Metric"],
                             y=metrics_df["Random Baseline"], marker_color="#9E9E9E",
    ...
    with col2:
        ...
        fig = px.imshow([[1523311, 25430], [555, 214]],
    ...
    with col1:
        ...
        shap_df = pd.DataFrame({
            "feature": ["rule_score", "amount_log", "payment_type_risk",
                        "sender_amount_sum_1h", "amount_vs_sender_avg",
                        "sender_avg_amount", "sender_tx_count_1h",
                        "is_structuring", "amount", "is_cross_currency",
                        "is_high_risk_type", "tx_hour", "is_round_amount",
                        "tx_day_of_week", "is_weekend"],
            "importance": [0.0842, 0.0731, 0.0698, 0.0621, 0.0589,
                           0.0534, 0.0498, 0.0445, 0.0412, 0.0389,
                           0.0334, 0.0298, 0.0267, 0.0234, 0.0198]
    ...
    if dist.get("histogram"):
        ...
        with col3:
            st.metric("Est. Recall", f"{tp_rate*100:.1f}%",
        ...
        fig.add_vline(x=sim_threshold, line_dash="dash", line_color="yellow",
                      annotation_text=f"Current: {sim_threshold:.2f}")
        fig.update_layout(title="Precision-Recall Tradeoff by Threshold",
                          xaxis_title="Threshold", yaxis_title="Score", height=350)
        st.plotly_chart(fig, use_container_width=True)
...
# ─────────────────────────────────────────
    ...
    if st.button("🔍 Search", type="primary") and tx_id:
        ...
        if not tx:
            ...
            risk_colors = {"CRITICAL": "error", "HIGH": "warning",
            ...
            with col1:
                st.metric("ML Probability",
                          ...
                          if tx.get('ml_probability') else "N/A")
            with col2:
                st.metric("Rule Score",
                          f"{tx.get('rule_score', 0):.4f}"
                          if tx.get('rule_score') else "N/A")
            with col3:
                st.metric("Amount", f"${tx.get('amount', 0):,.2f}")
            with col4:
                st.metric("Known Laundering",
                          "⚠️ YES" if tx.get("is_laundering") else "✅ NO")
            ...
            if alert:
                ...
                with col3:
                    ...
                    color = {"OPEN": "🔴", "INVESTIGATING": "🟠", "CLOSED": "🟢"}.get(status, "⚪")
                ...
                new_status = st.selectbox(
                    ...
                    index=["OPEN", "INVESTIGATING", "CLOSED"].index(alert.get("status", "OPEN"))
                )
                if st.button("Update Alert", type="primary"):
                    result = api_patch(f"/alerts/{alert.get('alert_id')}",
            ...
            fig = go.Figure(go.Indicator(
                mode="gauge+number+delta", value=ml_prob,
                domain={"x": [0, 1], "y": [0, 1]},
                title={"text": "ML Risk Score"}, delta={"reference": 0.9},
                gauge={
                    "axis": {"range": [0, 1]},
                    "bar": {"color": "#2196F3"},
                    "steps": [
        ...
        with st.form("predict_form"):
            ...
            with col3:
                sender_tx_count_1h = st.number_input("Sender TX Count 1h", value=3, min_value=0)
                sender_amount_sum_1h = st.number_input("Sender Amount Sum 1h", value=28500.0)
                sender_avg_amount = st.number_input("Sender Avg Amount", value=9500.0)
                amount_vs_sender_avg = st.number_input("Amount vs Sender Avg", value=1.0)

            col1, col2 = st.columns(2)
            with col1:
                payment_type_risk = st.number_input("Payment Type Risk", value=0.5,
                                                     min_value=0.0, max_value=1.0)
                is_high_risk_type = st.selectbox("High Risk Type", [0, 1])
            with col2:
                rule_score = st.number_input("Rule Score", value=0.5,
                                              min_value=0.0, max_value=1.0)
        ...
        if submitted:
            payload = {
                "amount": amount, "amount_log": amount_log,
                "tx_hour": tx_hour, "tx_day_of_week": tx_day_of_week,
                "is_weekend": is_weekend, "is_cross_currency": is_cross_currency,
