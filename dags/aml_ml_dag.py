...
def emit_lineage(input_dataset, output_dataset, run_id, job_name,
                 input_namespace='postgres', output_namespace='mlflow',
                 event_type='COMPLETE'):
...
def prepare_dataset(**context):
    ...
    emit_lineage('transactions_featured', 'ml/dataset', run_id, job_name,
                 event_type='START')

    feature_cols = [
        'amount', 'amount_log',
        'tx_hour', 'tx_day_of_week', 'is_weekend',
        'is_cross_currency', 'is_laundering',
        'sender_tx_count_1h', 'sender_amount_sum_1h',
        'sender_avg_amount', 'amount_vs_sender_avg',
        'payment_type_risk', 'is_high_risk_type',
        'is_structuring', 'is_round_amount', 'rule_score',
        'timestamp'
    ]
    ...
    logger.info(f"Train: {len(df_train):,} rows, laundering: {df_train['is_laundering'].sum():,}")
    logger.info(f"Val:   {len(df_val):,} rows, laundering: {df_val['is_laundering'].sum():,}")
    logger.info(f"Test:  {len(df_test):,} rows, laundering: {df_test['is_laundering'].sum():,}")
    ...
    log_audit(pg_conn, dag_id, run_id, task_id, 'ml', 'success',
              rows_processed=len(df), duration_seconds=round(duration, 2))
    emit_lineage('transactions_featured', 'ml/dataset', run_id, job_name,
                 event_type='COMPLETE')
    ...
    logger.info(f"Dataset ready. scale_pos_weight={scale_pos_weight} in {duration:.1f}s")
    return len(df)
...
def train_model(**context):
    ...
    s3 = get_s3_client()
    scale_pos_weight = context['ti'].xcom_pull(task_ids='prepare_dataset', key='scale_pos_weight')

    emit_lineage('ml/dataset', 'mlflow/aml_risk_model', run_id, job_name,
                 input_namespace='minio', output_namespace='mlflow', event_type='START')
    ...
    with mlflow.start_run(run_name=f'xgboost_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}') as mlrun:
    ...
    duration = time.time() - start_time
    emit_lineage('ml/dataset', 'mlflow/aml_risk_model', run_id, job_name,
                 input_namespace='minio', output_namespace='mlflow', event_type='COMPLETE')
...
def evaluate_model(**context):
    ...
    emit_lineage('mlflow/aml_risk_model', 'mlflow/aml_risk_model', run_id, job_name,
                 input_namespace='mlflow', output_namespace='mlflow', event_type='START')
    ...
    duration = time.time() - start_time
    emit_lineage('mlflow/aml_risk_model', 'mlflow/aml_risk_model', run_id, job_name,
                 input_namespace='mlflow', output_namespace='mlflow', event_type='COMPLETE')
    ...
    context['ti'].xcom_push(key='shap_importance', value=shap_importance.to_dict('records'))
...
def register_model(**context):
    ...
    threshold    = context['ti'].xcom_pull(task_ids='train_model', key='best_threshold')
    FEATURES     = context['ti'].xcom_pull(task_ids='train_model', key='features')
    test_auc_roc = context['ti'].xcom_pull(task_ids='evaluate_model', key='test_auc_roc')
    ...
    emit_lineage('mlflow/aml_risk_model', 'ml/scores', run_id, job_name,
                 input_namespace='mlflow', output_namespace='minio', event_type='START')
    ...
    client.set_model_version_tag("aml_risk_model", mv.version, "threshold", str(threshold))
    client.set_model_version_tag("aml_risk_model", mv.version, "test_auc_roc", str(round(test_auc_roc, 4)))
    client.set_model_version_tag("aml_risk_model", mv.version, "test_recall", str(round(test_recall, 4)))
    client.set_model_version_tag("aml_risk_model", mv.version, "test_f1", str(round(test_f1, 4)))
    ...
    logger.info(f"Saved: gold/ml/scores.parquet ({total_scored:,} rows, {buf.tell()/1e6:.1f} MB)")
    del df_scores, buf

    duration = time.time() - start_time
    log_audit(pg_conn, dag_id, run_id, task_id, 'ml', 'success',
              rows_processed=total_scored, duration_seconds=round(duration, 2))
    emit_lineage('mlflow/aml_risk_model', 'ml/scores', run_id, job_name,
                 input_namespace='mlflow', output_namespace='minio', event_type='COMPLETE')

    pg_conn.close()

    logger.info(f"Model registered & {total_scored:,} transactions scored in {duration:.1f}s")
