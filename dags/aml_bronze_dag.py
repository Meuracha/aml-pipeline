...
def emit_lineage(input_dataset, output_dataset, run_id, job_name,
                 input_namespace='local', output_namespace='minio',
                 event_type='COMPLETE'):
...
def upload_to_bronze(**context):
    ...
    emit_lineage('data/raw/LI-Small_Trans.csv', 'bronze/ibm_aml', run_id, job_name,
                 input_namespace='local', output_namespace='minio', event_type='START')
    ...
    try:
        for chunk in pd.read_csv(csv_path, chunksize=chunksize):
            chunk.columns = [
                'Timestamp', 'From Bank', 'Account_sender',
                'To Bank', 'Account_receiver',
            ...
            for (year, month), group in chunk.groupby(['_year', '_month']):
                ...
                partition_counts[partition_key] = partition_counts.get(partition_key, 0) + len(group)
        ...
        log_audit(pg_conn, dag_id, run_id, task_id, 'bronze', 'success',
                  rows_processed=total_rows, duration_seconds=round(duration, 2))
        emit_lineage('data/raw/LI-Small_Trans.csv', 'bronze/ibm_aml', run_id, job_name,
                     input_namespace='local', output_namespace='minio', event_type='COMPLETE')

    except Exception as e:
        duration = time.time() - start_time
        log_audit(pg_conn, dag_id, run_id, task_id, 'bronze', 'failed',
                  rows_processed=total_rows, duration_seconds=round(duration, 2),
                  error_message=str(e))
        emit_lineage('data/raw/LI-Small_Trans.csv', 'bronze/ibm_aml', run_id, job_name,
                     input_namespace='local', output_namespace='minio', event_type='FAIL')
        raise
...
def validate_bronze(**context):
    ...
    emit_lineage('bronze/ibm_aml', 'bronze/ibm_aml', run_id, job_name,
                 input_namespace='minio', output_namespace='minio', event_type='START')
    ...
    try:
        ...
        required_cols = [
            'Timestamp', 'From Bank', 'Account_sender', 'To Bank',
            'Account_receiver', 'Amount Paid', 'Is Laundering', 'source'
        ]
        ...
        log_audit(pg_conn, dag_id, run_id, task_id, 'bronze', 'success',
                  rows_processed=total_rows, duration_seconds=round(duration, 2))
        emit_lineage('bronze/ibm_aml', 'bronze/ibm_aml', run_id, job_name,
                     input_namespace='minio', output_namespace='minio', event_type='COMPLETE')
        ...
        log_audit(pg_conn, dag_id, run_id, task_id, 'bronze', 'failed',
                  ...
                  error_message=str(e))
        emit_lineage('bronze/ibm_aml', 'bronze/ibm_aml', run_id, job_name,
                     input_namespace='minio', output_namespace='minio', event_type='FAIL')
        raise
...
def route_dead_letter(**context):
    ...
    emit_lineage('bronze/ibm_aml', 'dead-letter/ibm_aml', run_id, job_name,
                 input_namespace='minio', output_namespace='minio', event_type='START')
    ...
    try:
        ...
        duration = time.time() - start_time
        log_audit(pg_conn, dag_id, run_id, task_id, 'bronze', 'success',
                  rows_dead_letter=dead_letter_rows, duration_seconds=round(duration, 2))
        emit_lineage('bronze/ibm_aml', 'dead-letter/ibm_aml', run_id, job_name,
                     input_namespace='minio', output_namespace='minio', event_type='COMPLETE')
